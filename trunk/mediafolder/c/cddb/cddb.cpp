...
void cddb::addTrack(clsTrack * track)
{
  ...
  /* Check if this is another part of title information spawned over several lines */
  tempTrack=cddbFindTrack(track->iTrackNo);
  if(tempTrack) {
    /* Yes, add  the rest of the title to that what we already have */
    strncat(tempTrack->trackname, track->trackname, sizeof(tempTrack->trackname)-strlen(tempTrack->trackname)-1);    
    delete(track);
    return;
  }
...
/* This method finds the track with the specified num (starting with 0) */
clsTrack* cddb::cddbFindTrack(int iTrack)
{
  clsTrack * tempTrack;

  tempTrack=firstTrack;
  while(tempTrack) {
    if(tempTrack->iTrackNo==iTrack)
      return tempTrack;
...
#if 0
/* now defined in printHelper.cpp 
   or cddbhelper.cpp. By choosing the right module to link to, it's possible
   to redirect the messages without changing this module. Used for different
   output with querycddb.exe and pmcddb.exe. */
...
cddb * cddb_query(int s, CDDBINFO *cddbInfo)
{
  ...
  for(a=0;a<cddbInfo->numTracks;a++) {
    rc=snprintf(track,sizeof(track)," %d",cddbInfo->startSector[a]);
    if(rc==EOF||rc>=sizeof(track)) {
      printError("Query: buffer overrun while formatting track offsets.\nPlease report this bug.\n");
      return 0;
    }
    if(sizeof(buffer)-strlen(buffer) <= strlen(track)) {
      printError("Query: buffer overrun while adding track offsets to command.\nPlease report this bug.\n");
      return 0;
    }
    strncat(buffer,track, sizeof(buffer)-strlen(buffer));
  }

  rc=snprintf(track,sizeof(track), " %d\n",cddbInfo->discLength);
  if(rc==EOF||rc>=sizeof(track)) {
    printError("Query: buffer overrun while formatting disclength.\nPlease report this bug.\n");
    return 0;
  }
  if(sizeof(buffer)-strlen(buffer) <= strlen(track)) {
    printError("Query: buffer overrun while adding disclength to command.\nPlease report this bug.\n");
    return 0;
  }
  strncat(buffer,track, sizeof(buffer)-strlen(buffer));
...
int read_and_parse(int s,cddb * Cddb)
{
  ...
  do{
    ...
    /* Print the data to the outfile. The terminating '.' isn't printed out so the datafile
       is usable for a cddb commit. */
    if(buffer[0]!='.')
      printData(buffer);
