...
HWND hwndVideo, hwndVideoClient;
    ...
    case WM_INITDLG:
      {
        ...
        createFlags=FCF_TITLEBAR;
        hwndVideo=WinCreateStdWindow(HWND_DESKTOP, WS_VISIBLE, &createFlags, WC_STATIC, "Video", 0, NULLHANDLE, 123, &hwndVideoClient);
        WinSetWindowPos(hwndVideo, NULLHANDLE, 0, 0, 300, 200, SWP_SIZE|SWP_SHOW);
  ...
  if(DosGetInfoBlocks(&ptib, &ppib)==NO_ERROR)
    {
      printf("Got info block...\n");
      ulPid=ppib->pib_ulpid;
      ppib->pib_ultype=2; /* Morph into VIO */
    }
