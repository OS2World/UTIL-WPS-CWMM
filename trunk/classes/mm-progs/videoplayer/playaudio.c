...
extern HWND hwndVideo, hwndVideoClient;
...
ULONG playAudioFile(HWND hwndFrame)
{
  ...
  hwndNotify=hwndFrame;//WinWindowFromID(hwndTop, IDDLG_TOPMIDDLE);
    ...
    sprintf(chrCommand,"open \"%s\"  type %s alias wave%d SHAREABLE wait",
            chrSourceName, chrDevice, iWavePriv);

    SysWriteToTrapLog("Here %s\n %s\n", chrSourceName, chrCommand);
    
    //  sprintf(chrCommand,"open digitalvideo01 alias vid  wait", chrSourceName, iWavePriv);
...
void _Optlink playThreadFunc (void *arg)
{
  ...
  if(hab) {
    ...
    if(hmq) {
      ...
      hwndPlayObject=WinCreateWindow(HWND_OBJECT,WC_STATIC,"MFObj",0,0,0,100,100,NULLHANDLE,HWND_BOTTOM,13343,NULL,NULL);
      if(hwndPlayObject) {
