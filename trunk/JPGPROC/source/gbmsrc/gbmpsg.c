	...
	while ( gbm_file_read(fd, buf, sizeof(buf)) == sizeof(buf) )
		...
		if ( psg_priv->fix_badrec )
			for ( f = 0; buf[0] != 0x5a && f < 10; f++ )
				{
				long pos = gbm_file_lseek(fd, 0L, SEEK_CUR);
				if ( pos == 0L )
					break;
				gbm_file_lseek(fd, pos - (sizeof(buf)+1), SEEK_SET);
		...
		seek_by = (long) getword(buf+1)-(2+6);
			...
			{
			psg_priv->pos = gbm_file_lseek(fd, - (long) sizeof(buf), SEEK_CUR);
			return gbm->w != -1 ? GBM_ERR_OK : GBM_ERR_PSEG_SIZE;
			}
		else
			return GBM_ERR_PSEG_BAD_RECORD;
		gbm_file_lseek(fd, seek_by, SEEK_CUR);
		}
...
GBM_ERR psg_rdata(int fd, GBM *gbm, byte *data)
	...
	gbm_file_lseek(fd, psg_priv->pos, SEEK_SET);
...
/*...spsg_w:0:*/
/*...srecord:0:*/
static BOOLEAN record(int fd, byte *rec, char *six, int len)
	{
	rec[0] = 0x5a;
...
GBM_ERR psg_w(const char *fn, int fd, const GBM *gbm, const GBMRGB *gbmrgb, const byte *data, const char *opt)
	...
	putword(r+2, pelx); /* Or 0? */
	putword(r+4, pely); /* Or 0? */
	putword(r+6+ 8, pelx);
	putword(r+6+10, pely);
