	...
	if ( tif_priv->spp != 1 )
		...
		if ( (tag_cm = locate_tag(ifd, T_COLORMAP)) != NULL )
			...
			for ( i = 0, gbmrgb = tif_priv->gbmrgb; i < n_pal; i++, gbmrgb++ )
				{
				gbmrgb->r = (byte) ((unsigned)value_of_tag_n(tag_cm,             i) >> 8);
				gbmrgb->g = (byte) ((unsigned)value_of_tag_n(tag_cm,     n_pal + i) >> 8);
				gbmrgb->b = (byte) ((unsigned)value_of_tag_n(tag_cm, 2 * n_pal + i) >> 8);
				}
...
case ENC_NONE:
	return ( (int) n_bytes == gbm_file_read(fd, dest, (int) n_bytes) ) ?
...
if ( (buf = malloc((size_t) n_bytes)) == NULL )
	return GBM_ERR_MEM;
}
/*...e*/
...
if ( (buf = malloc((size_t) (n_bytes * 3))) == NULL )
	return GBM_ERR_MEM;
p[0] = buf;
p[1] = p[0] + n_bytes;
p[2] = p[1] + n_bytes;
...
/*...suser_tag:0:*/
static BOOLEAN user_tag(IFD *ifd, char *name, short type, const char *opt, char *def)
	{
	char buf[200+1];
	const char *s;
...
/*...e*/
/*...swrite_strip:0:*/
static GBM_ERR write_strip(int fd, int w, int h, int bpp, const byte *data)
	{
...
static GBM_ERR write_strip_lzw(int fd, int w, int h, int bpp, const byte *data)
	{
	int stride = ((bpp * w + 31) / 32) * 4;
	int ostride = ((bpp * w + 7) / 8);
	WRITE_CONTEXT wc;
...
GBM_ERR tif_w(const char *fn, int fd, const GBM *gbm, const GBMRGB *gbmrgb, const byte *data, const char *opt)
	{
	BOOLEAN	baseline = ( gbm_find_word(opt, "pal1bpp") == NULL );
	BOOLEAN lzw      = ( gbm_find_word(opt, "lzw"    ) != NULL );
	IFH	*ifh;
	IFD	*ifd;
	long	w = gbm->w;
	long	h = gbm->h;
