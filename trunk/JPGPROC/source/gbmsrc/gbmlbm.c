...
priv->sham = gbm_file_lseek(fd, 0L, SEEK_CUR);

gbm_file_lseek(fd, ((size - 2 + 1) & ~1), SEEK_CUR);
}
/*...e*/
...
priv->body = (dword) gbm_file_lseek(fd, 0L, SEEK_CUR);
priv->size_body = size;
had_body = TRUE;
}
/*...e*/
		else
			gbm_file_lseek(fd, ((size + 1) & ~1), SEEK_CUR);
		}
...
GBM_ERR lbm_rdata(int fd, GBM *gbm, byte *data)
	...
	gbm_file_lseek(fd, priv->body, SEEK_SET);
...
gbm_file_lseek(fd, priv->sham, SEEK_SET);
...
static GBM_ERR write_body(int fd, const GBM *gbm, int bpp, int n_planes, const byte *data, long *end)
	...
	offset_body = gbm_file_lseek(fd, 0L, SEEK_CUR);
...
/*...e*/
	...
	offset_end = gbm_file_lseek(fd, 0L, SEEK_CUR);
	...
	gbm_file_lseek(fd, offset_body - 4L, SEEK_SET);
